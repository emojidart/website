...
const autoResolveFreilosMatch = (allMatches: Record<number, Match>, matchId: number) => {
  ...
  const openBeamer = () => {
    ...
    window.open(
      `/16erdko/beamer?${params.toString()}`,
      "dko-beamer-16",
      ...
      <div className="mx-auto w-full max-w-[1920px] space-y-5 px-3 pb-8 pt-3 sm:px-5 lg:px-7 xl:px-8">
        <div className="flex flex-col gap-3 rounded-[22px] border border-slate-200 bg-white px-4 py-3 shadow-[0_14px_40px_-32px_rgba(15,23,42,.55)] sm:flex-row sm:items-center sm:justify-between sm:px-5">
          ...
          <div className="flex flex-wrap items-center gap-2 sm:justify-end">
            <div className="flex h-9 items-center gap-2 rounded-xl border border-slate-200 bg-slate-50 px-3">
              <Checkbox
                id="announcements"
                checked={announcementsEnabled}
                onCheckedChange={(checked) => setAnnouncementsEnabled(checked === true)}
              />
              <Label htmlFor="announcements" className="flex cursor-pointer items-center gap-1.5 text-sm font-bold text-slate-700">
