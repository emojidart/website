    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-md max-h-[80vh] overflow-y-auto">
        ...
        <div className="space-y-4 mt-4">
          {/* Total Points */}
          <div className="bg-gradient-to-r from-amber-50 to-yellow-50 rounded-lg p-3 border-2 border-amber-200">
            <div className="text-center">
              <div className="text-xs text-gray-600 mb-1">Gesamtpunkte</div>
              <div className="text-3xl font-bold text-amber-600">{pointsBreakdown.totalPoints.toFixed(1)}</div>
            </div>
          </div>
          ...
          <div className="space-y-2">
            ...
            <div className="bg-green-50 rounded-lg p-2 flex justify-between items-center">
              <span className="text-xs">{player.total_wins} Siege × 3 Punkte</span>
              <Badge className="bg-green-600 text-white font-bold text-xs">{pointsBreakdown.legWinPoints} Punkte</Badge>
            </div>
          </div>
            ...
            <div className="grid grid-cols-1 gap-2">
              {pointsBreakdown.throw180Points > 0 && (
                <div className="bg-red-50 rounded-lg p-2 flex justify-between items-center">
                  <span className="text-xs">{player.throws_180} × 180er (25 Punkte)</span>
                  <Badge className="bg-red-600 text-white font-bold text-xs">
                    {pointsBreakdown.throw180Points} Punkte
                  </Badge>
                </div>
              )}
              {pointsBreakdown.throw171Points > 0 && (
                <div className="bg-purple-50 rounded-lg p-2 flex justify-between items-center">
                  <span className="text-xs">{player.throws_171} × 171er (25 Punkte)</span>
                  <Badge className="bg-purple-600 text-white font-bold text-xs">
                    {pointsBreakdown.throw171Points} Punkte
                  </Badge>
                </div>
              )}
              {pointsBreakdown.highTonnePoints > 0 && (
                <div className="bg-orange-50 rounded-lg p-2 flex justify-between items-center">
                  <span className="text-xs">{player.throws_high_tonne} × High Tonne (18 Punkte)</span>
                  <Badge className="bg-orange-600 text-white font-bold text-xs">
                    {pointsBreakdown.highTonnePoints} Punkte
                  </Badge>
                </div>
              )}
              {pointsBreakdown.tonnePoints > 0 && (
                <div className="bg-green-50 rounded-lg p-2 flex justify-between items-center">
                  <span className="text-xs">{player.throws_tonne} × Tonne (15 Punkte)</span>
                  <Badge className="bg-green-600 text-white font-bold text-xs">
                    {pointsBreakdown.tonnePoints} Punkte
                  </Badge>
                </div>
              )}
              {pointsBreakdown.throw95PlusPoints > 0 && (
                <div className="bg-teal-50 rounded-lg p-2 flex justify-between items-center">
                  <span className="text-xs">{player.throws_95_plus} × 95+ (12 Punkte)</span>
                  <Badge className="bg-teal-600 text-white font-bold text-xs">
                    {pointsBreakdown.throw95PlusPoints} Punkte
                  </Badge>
                </div>
              )}
              {pointsBreakdown.shanghaiPoints > 0 && (
                <div className="bg-indigo-50 rounded-lg p-2 flex justify-between items-center">
                  <span className="text-xs">{player.throws_shanghai} × Shanghai (10 Punkte)</span>
                  <Badge className="bg-indigo-600 text-white font-bold text-xs">
                    {pointsBreakdown.shanghaiPoints} Punkte
                  </Badge>
                </div>
              )}
              <div className="bg-pink-50 rounded-lg p-2 flex justify-between items-center">
                <span className="text-xs">{player.throws_bull || 0} × Bull (8 Punkte)</span>
                <Badge className="bg-pink-600 text-white font-bold text-xs">{pointsBreakdown.bullPoints} Punkte</Badge>
              </div>
            </div>
          </div>
