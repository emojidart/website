  ...
  useEffect(() => {
    const loadActiveTournament = async () => {
      try {
        ...
        if (error) {
          console.log("[v0] No active tournament found")
          setLoading(false)
          return
        }

        if (data) {
          console.log("[v0] Active tournament found:", data)
          setActiveTournament({
        ...
        console.error("[v0] Error loading active tournament:", error)
        setLoading(false)
      }
    }
    ...
    const channel = supabase
      ...
      .on(
        ...
        (payload) => {
          console.log("[v0] Tournament status changed:", payload)
          if (payload.eventType === "INSERT" || payload.eventType === "UPDATE") {
            ...
            if (data.status === "active") {
              ...
              if (activeTournament?.tournament_id === data.tournament_id) {
        ...
        <main className="pt-8 pb-20">
          <div className="container mx-auto px-4 md:px-6 py-8">
            <div className="text-center">
              ...
              <p className="mt-4 text-gray-600">Lade Turnier-Daten...</p>
            </div>
          </div>
        </main>
      </div>
                    ...
                    {completedMatches.map((match) => (
                      <div key={match.id} className="border border-gray-200 rounded-xl p-4 bg-white shadow-sm">
                        ...
                        <div className="space-y-2">
                          ...
                          >
                            <span
                              className={cn("text-sm", match.winner === match.player1 && "font-bold text-gray-900")}
                            >
                              {match.player1}
                            </span>
                            <span
                              className={cn(
                                ...
                                match.winner === match.player1 && "text-green-600",
                              )}
                            >
                              {match.score1}
                            </span>
                          ...
                          >
                            <span
                              className={cn("text-sm", match.winner === match.player2 && "font-bold text-gray-900")}
                            >
                              {match.player2}
                            </span>
                            <span
                              className={cn(
                                "text-sm font-semibold",
                                match.winner === match.player2 && "text-green-600",
                              )}
                            >
                              {match.score2}
                            </span>
