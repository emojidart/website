...
const autoResolveFreilosMatch = (allMatches: Record<number, Match>, matchId: number) => {
  ...
  const matchCenterReadyMatches = readyMatches.slice(0, 8)
  ...
  const openBeamer = () => {
    ...
    window.open(
      `/64erdko/beamer?${params.toString()}`,
      "dko-beamer-64",
                ...
                <div className="grid gap-3">
                  {activeLiveMatches.map((match) => (
                    <MatchCard
                      ...
                      isGrandFinal={match.id === 126 || match.id === 127}
                    />
                  ))}
                </div>
              ) : (
            ...
            <div className="min-w-0">
              <div className="mb-3 flex items-center justify-between gap-3">
                ...
                <span className="text-xs font-bold text-slate-400">
                  {Math.min(matchCenterReadyMatches.length, 8)} von {readyMatches.length}
                </span>
              </div>
