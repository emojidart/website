...
// Inserts a new round's board configuration with proper status tracking
export async function saveKratzerTournamentRound(
  ...
  try {
    // Ensure board data includes proper status and timing information
    const processedBoardsData = boardsData.map((board) => ({
      ...board,
      status: board.status || (board.startTime ? "running" : "not_started"),
      endTime: board.endTime || null,
      timer: null, // Remove timer from serialization
    }))

    const { error } = await supabase.from("kratzer_tournament_rounds").insert({
      kratzer_tournament_id: tournamentId,
      round_number: roundNumber,
      boards_data: JSON.parse(JSON.stringify(processedBoardsData)), // Deep copy to ensure no reactivity issues
    })
...
// Updates the status of a specific board in the current round
export async function updateBoardStatus(
  ...
  try {
    // Get the latest round for this tournament
    ...
    if (roundError || !latestRound) {
      throw new Error("Keine aktuelle Runde gefunden")
    }

    // Update the specific board in the boards_data
    const updatedBoardsData = latestRound.boards_data.map((board: any) => {
      if (board.id === boardId) {
        const updatedBoard = { ...board, status }
        if (startTime !== undefined) updatedBoard.startTime = startTime
        if (endTime !== undefined) updatedBoard.endTime = endTime
        return updatedBoard
      }
      return board
    })

    // Update the round with the new board data
    const { error: updateError } = await supabase
      .from("kratzer_tournament_rounds")
      .update({ boards_data: updatedBoardsData })
    ...
    return { success: true, message: "Board-Status erfolgreich aktualisiert." }
  } catch (error: any) {
    console.error("Fehler beim Aktualisieren des Board-Status:", error)
    return {
      success: false,
      message: `Fehler beim Aktualisieren des Board-Status: ${error.message}`,
    }
  }
}
