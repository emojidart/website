...
export async function POST(request: NextRequest) {
  try {
    const { tournamentId, name, date, photo_url } = await request.json()
    ...
    const payload = JSON.stringify({
      ...
      image: photo_url,
      badge: "/icon-192.png",
      data: { link: `/tournament-series/${tournamentId}` },
    })
    ...
    return NextResponse.json({
      success: true,
      message: `Tournament notification sent to ${successCount} devices`,
      successCount,
      failureCount,
    })
  } catch (error) {
    console.error("[v0] Error sending tournament push:", error)
