...
export async function POST(request: NextRequest) {
  try {
    ...
    // ✅ Europe/Vienna/Berlin: wir nehmen "heute" über Locale (einfach & stabil)
    const now = new Date()
    const year = now.getFullYear()
    const month = now.getMonth() + 1
    const day = now.getDate()

    // 1) Spieler mit Geburtstag heute holen
    ...
    // 2) alle Empfänger: alle user_ids die FCM Tokens haben
    ...
    if (tokErr) {
      ...
      return NextResponse.json({ success: false, error: "token fetch failed" }, { status: 500 })
    }
    ...
    // 3) pro Geburtstagskind: prüfen ob dieses Jahr schon gesendet wurde
    ...
    for (const p of todayBirthdayPlayers as any[]) {
      ...
      if (already?.id) continue // ✅ dieses Jahr schon gesendet
      ...
      const clickUrl = `/vereinskalender-app` // oder /vereinskalender-app?tab=geburtstage
