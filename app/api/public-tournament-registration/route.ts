...
export async function POST(request: Request) {
  try {
    ...
    if (resendKey) {
      ...
      await fetch("https://api.resend.com/emails", { method:"POST", headers:{ Authorization:`Bearer ${resendKey}`, "Content-Type":"application/json" }, body:JSON.stringify({ from, to:email, subject:`Anmeldebestätigung – ${event.name}`, html, text:`Du bist für ${event.name} am ${date} angemeldet.` }) })
    }
