...
export default function AdminPage() {
  ...
  const autoAssignDivisions = async () => {
    ...
    try {
      ...
      toast.success(`Divisionen automatisch zugewiesen! ${halfPoint} Spieler in A, ${totalPlayers - halfPoint} Spieler in B`)
    } catch (error) {
  ...
  const updatePlayerDivision = async (playerId: string, newDivision: string | null) => {
    ...
    try {
      ...
      setPlayers((prev) =>
        prev.map((p) => (p.player_id === playerId ? { ...p, division: newDivision } : p))
      )
            ...
            <Card className="border-2 border-purple-200">
              ...
              <CardContent className="space-y-4">
                  ...
                  <div>
                    ...
                    <div className="text-sm text-gray-600">
                      {settings.division_active
                        ? "A/B Tabellen sind aktiv"
                        : "Gesamttabelle wird angezeigt"}
                    </div>
                  </div>
                  <Switch 
                    checked={settings.division_active} 
                    onCheckedChange={toggleDivision} 
                    disabled={saving} 
                  />
                </div>
                  ...
                  <Alert className="border-purple-200 bg-purple-50">
                    ...
                    <AlertDescription className="text-purple-900">
                      ...
                      <div className="mt-1 text-sm">
                        Spieler können nicht mehr zwischen A und B wechseln.
                      </div>
                    </AlertDescription>
                  </Alert>
                )}
                ...
                <div className="space-y-2">
                  <Button
                    onClick={autoAssignDivisions}
                    disabled={assigningDivisions || players.length === 0}
                    className="w-full"
                  >
                    {assigningDivisions ? (
          ...
          <div className="space-y-6">
                ...
                <div className="max-h-[300px] overflow-y-auto">
                  {playersInA.length === 0 ? (
                    <div className="p-6 text-center text-gray-500">
                      Noch keine Spieler in Tabelle A
                    </div>
                  ) : (
            ...
            <Card className="border-2 border-slate-300">
              <CardHeader className="bg-slate-50 border-b border-slate-200">
                <CardTitle className="flex items-center gap-2 text-slate-900">
                  ...
                  Tabelle B - Platzierungsrunde
                  <Badge variant="secondary" className="ml-auto">{playersInB.length} Spieler</Badge>
                </CardTitle>
              ...
              <CardContent className="p-0">
                <div className="max-h-[300px] overflow-y-auto">
                  {playersInB.length === 0 ? (
                    <div className="p-6 text-center text-gray-500">
                      Noch keine Spieler in Tabelle B
                    </div>
                  ) : (
                    <table className="w-full">
              ...
              <Card className="border-2 border-dashed border-gray-300">
                <CardHeader className="bg-gray-50 border-b border-gray-200">
                  <CardTitle className="flex items-center gap-2 text-gray-700">
                    <Users className="h-5 w-5" />
                    Nicht zugeteilt
                    <Badge variant="outline" className="ml-auto">{playersUnassigned.length} Spieler</Badge>
                  </CardTitle>
            ...
            <Alert className="border-gray-200 bg-gray-50">
              ...
              <AlertDescription className="text-gray-700 text-sm">
                <strong>"Alle Divisionen zurücksetzen"</strong> löscht nur die A/B Zuordnung der Spieler. 
                Alle Turnierdaten, Punkte und Ergebnisse bleiben vollständig erhalten!
              </AlertDescription>
            </Alert>
          </div>
        </div>
