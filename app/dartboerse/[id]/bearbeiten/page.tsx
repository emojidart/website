...
export default function DartboerseBearbeitenPage(){
 ...
 if(loading)return <div className="min-h-screen bg-slate-50"><Header/><div className="pt-32 text-center"><Loader2 className="mx-auto h-10 w-10 animate-spin text-orange-600"/></div></div>
 if(!form)return <div className="min-h-screen bg-slate-50"><Header/><main className="px-4 pt-24"><Card className="mx-auto max-w-lg rounded-3xl"><CardContent className="p-8 text-center"><ShieldAlert className="mx-auto h-10 w-10 text-orange-600"/><p className="mt-4 font-bold">{message}</p></CardContent></Card></main></div>
 return <div className="min-h-screen bg-slate-50 pb-24"><Header/><main className="mx-auto max-w-4xl px-4 pt-20"><Button variant="outline" onClick={()=>router.push("/dartboerse/meine")} className="mb-4 rounded-xl"><ArrowLeft className="mr-2 h-4 w-4"/>Zurück</Button><section className="rounded-[2rem] bg-gradient-to-br from-slate-950 to-orange-950 p-6 text-white shadow-2xl"><div className="text-sm font-black uppercase tracking-[.18em] text-orange-300">Bearbeiten</div><h1 className="mt-2 text-3xl font-black">Inserat ändern</h1><p className="mt-2 text-slate-300">Nach dem Speichern wird das Angebot erneut geprüft.</p></section><form onSubmit={save} className="mt-5 space-y-5"><Card className="rounded-3xl"><CardContent className="space-y-4 p-6"><Field label="Titel"><Input value={form.title} onChange={e=>setField("title",e.target.value)} required/></Field><div className="grid gap-4 sm:grid-cols-2"><Field label="Kategorie"><Select value={form.category} onValueChange={v=>setField("category",v)}><SelectTrigger><SelectValue/></SelectTrigger><SelectContent>{Object.entries(categories).map(([v,l])=><SelectItem key={v} value={v}>{l}</SelectItem>)}</SelectContent></Select></Field><Field label="Zustand"><Select value={form.condition} onValueChange={v=>setField("condition",v)}><SelectTrigger><SelectValue/></SelectTrigger><SelectContent><SelectItem value="new">Neu</SelectItem><SelectItem value="like_new">Wie neu</SelectItem><SelectItem value="good">Gut</SelectItem><SelectItem value="used">Gebraucht</SelectItem><SelectItem value="defective">Defekt</SelectItem></SelectContent></Select></Field></div><Field label="Beschreibung"><Textarea rows={7} value={form.description} onChange={e=>setField("description",e.target.value)}/></Field>
