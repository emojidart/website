...
export default function ProFAQChat() {
  const [messages, setMessages] = useState<Message[]>([
    {
      from: "bot",
      text: "Hallo! 👋 Ich bin dein EMD Chat-Assistent. Wie kann ich dir helfen?",
    },
  ])
  ...
  const getKeywords = (keywords: string[] | null | undefined) => {
    ...
    return keywords.map((k) => normalize(String(k))).filter(Boolean)
  }

  const simulateTyping = async (answer: string) => {
    const baseDelay = 500
    const typingSpeed = 20
    const delay = baseDelay + Math.min(answer.length * typingSpeed, 1200)
    await new Promise((resolve) => setTimeout(resolve, delay))
  }

  const findBestMatch = (query: string, faqs: FAQ[]) => {
    const exactMatch = faqs.find((faq) => {
      const normalizedQuestion = normalize(faq.question)
      const normalizedKeywords = getKeywords(faq.keywords)

      return normalizedQuestion === query || normalizedKeywords.includes(query)
    })

    if (exactMatch) return exactMatch

    const fallbackMatch = faqs.find((faq) => {
      const normalizedQuestion = normalize(faq.question)
      const normalizedKeywords = getKeywords(faq.keywords)

      const questionContains =
        normalizedQuestion.length > 0 &&
        (normalizedQuestion.includes(query) || query.includes(normalizedQuestion))

      const keywordContains = normalizedKeywords.some(
        (keyword) =>
          keyword.length > 0 &&
          (keyword.includes(query) || query.includes(keyword))
      )

      return questionContains || keywordContains
    })
  ...
  const handleAsk = async (q: string) => {
    ...
    let answer = "Das kann ich dir nicht beantworten. 🧐 Versuche es mit einer anderen Frage!"

    if (!error && data) {
      const matched = findBestMatch(query, data as FAQ[])
