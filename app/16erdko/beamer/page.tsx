...
function getBracketSide(matchId: number) {
  ...
  if ((matchId >= 9 && matchId <= 12) || (matchId >= 17 && matchId <= 22) || (matchId >= 25 && matchId <= 27) || matchId === 29) {
    return {
...
function WinnersView({ matches }: { matches: Record<number, DkoMatch> }) {
  ...
  const r1 = pick([1, 2, 3, 4, 5, 6, 7, 8])
  const r2 = pick([13, 14, 15, 16])
  const r3 = pick([23, 24])
  const r4 = pick([28])
  const count = r1.length + r2.length + r3.length + r4.length
  ...
  return (
    <section className="mt-5 rounded-[26px] border border-sky-300/10 bg-sky-400/[0.018] p-5">
      ...
      <div className="mt-5 grid gap-5 xl:grid-cols-4">
        <StageColumn label="Runde 1" matches={r1} side="winner" />
        <StageColumn label="Runde 2" matches={r2} side="winner" />
        <StageColumn label="Runde 3" matches={r3} side="winner" />
        <StageColumn label="Finale Gewinnerseite" matches={r4} side="winner" />
      </div>
    </section>
  )
}
...
function LosersView({ matches }: { matches: Record<number, DkoMatch> }) {
  ...
  const pick = (ids: number[]) => ids.filter(ready).map((id) => matches[id])
  const r1 = pick([9, 10, 11, 12])
  const r2 = pick([17, 18, 19, 20])
  const r3 = pick([21, 22])
  const r4 = pick([25, 26])
  const r5 = pick([27])
  const r6 = pick([29])
  const count = r1.length + r2.length + r3.length + r4.length + r5.length + r6.length
  ...
  return (
    <section className="mt-5 rounded-[26px] border border-rose-300/10 bg-rose-400/[0.018] p-5">
      ...
      <div className="mt-5 grid gap-5 xl:grid-cols-6">
        <StageColumn label="Runde 1" matches={r1} side="loser" />
        <StageColumn label="Runde 2" matches={r2} side="loser" />
        <StageColumn label="Runde 3" matches={r3} side="loser" />
        <StageColumn label="Runde 4" matches={r4} side="loser" />
        <StageColumn label="Runde 5" matches={r5} side="loser" />
        <StageColumn label="Finale Verliererseite" matches={r6} side="loser" />
      </div>
    </section>
  )
}
...
function FinalsView({ matches }: { matches: Record<number, DkoMatch> }) {
  const finalMatch = matches[30]
  const resetMatch = matches[31]
...
export default function DkoBeamerPage() {
  ...
  const tournamentName = searchParams.get("tournamentName") || "16er DKO"
  const tournamentType = searchParams.get("tournamentType") || "16er_dko"
  ...
  const winner =
    matches[31]?.winner ||
    (matches[30]?.winner && matches[30]?.winner === matches[30]?.player1 ? matches[30]?.winner : undefined)

  const hasLosersView = [9, 10, 11, 12, 17, 18, 19, 20, 21, 22, 25, 26, 27, 29].some((id) => {
    const match = matches[id]
    return Boolean(match && ((match.player1 && match.player2) || match.winner || match.machineNumber))
  })
  const hasFinalsView = [30, 31].some((id) => {
    const match = matches[id]
    return Boolean(match && ((match.player1 && match.player2) || match.winner || match.machineNumber))
  })
    ...
    return (
      <div className="grid min-h-screen place-items-center bg-slate-950 px-6 text-white">
        <div className="max-w-lg rounded-[28px] border border-white/10 bg-white/5 p-8 text-center">
          ...
          <p className="mt-2 text-slate-400">Öffne den Beamer direkt aus dem laufenden 16er-DKO.</p>
        </div>
      </div>
    )
  }
                  ...
                  {[
                    ...
                    const fallbackName =
                      slot.place === 1
                        ? winner
                        : slot.place === 2
                          ? (matches[31]?.loser || matches[30]?.loser || "")
                          : (matches[29]?.loser || "")
