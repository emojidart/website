...
function getBracketSide(matchId: number) {
  const winnerIds = new Set([
    ...
    ...Array.from({ length: 8 }, (_, i) => i + 217),
    ...Array.from({ length: 4 }, (_, i) => i + 237),
    ...Array.from({ length: 2 }, (_, i) => i + 247),
    252,
  ])

  const loserIds = new Set([
    ...Array.from({ length: 32 }, (_, i) => i + 65),
    ...Array.from({ length: 32 }, (_, i) => i + 129),
    ...Array.from({ length: 16 }, (_, i) => i + 161),
    ...Array.from({ length: 16 }, (_, i) => i + 193),
    ...Array.from({ length: 8 }, (_, i) => i + 209),
    ...Array.from({ length: 8 }, (_, i) => i + 225),
    ...Array.from({ length: 4 }, (_, i) => i + 233),
    ...Array.from({ length: 4 }, (_, i) => i + 241),
    ...Array.from({ length: 2 }, (_, i) => i + 245),
    ...Array.from({ length: 2 }, (_, i) => i + 249),
    251, 253,
  ])
...
function WinnersView({ matches }: { matches: Record<number, DkoMatch> }) {
  ...
  const stages = [
    { label: "Runde 1", ids: Array.from({ length: 64 }, (_, i) => i + 1) },
    { label: "Runde 2", ids: Array.from({ length: 32 }, (_, i) => i + 97) },
    { label: "Runde 3", ids: Array.from({ length: 16 }, (_, i) => i + 177) },
    { label: "Runde 4", ids: Array.from({ length: 8 }, (_, i) => i + 217) },
    { label: "Runde 5", ids: Array.from({ length: 4 }, (_, i) => i + 237) },
    { label: "Runde 6", ids: Array.from({ length: 2 }, (_, i) => i + 247) },
    { label: "Finale Gewinnerseite", ids: [252] },
  ]
  ...
  return (
    <section className="mt-5 rounded-[26px] border border-sky-300/10 bg-sky-400/[0.018] p-5">
      ...
      <div className="mt-5 grid gap-5 xl:grid-cols-4 2xl:grid-cols-7">
        {stages.map((stage) => (
...
function LosersView({ matches }: { matches: Record<number, DkoMatch> }) {
  ...
  const stages = [
    { label: "Runde 1", ids: Array.from({ length: 32 }, (_, i) => i + 65) },
    { label: "Runde 2", ids: Array.from({ length: 32 }, (_, i) => i + 129) },
    { label: "Runde 3", ids: Array.from({ length: 16 }, (_, i) => i + 161) },
    { label: "Runde 4", ids: Array.from({ length: 16 }, (_, i) => i + 193) },
    { label: "Runde 5", ids: Array.from({ length: 8 }, (_, i) => i + 209) },
    { label: "Runde 6", ids: Array.from({ length: 8 }, (_, i) => i + 225) },
    { label: "Runde 7", ids: Array.from({ length: 4 }, (_, i) => i + 233) },
    { label: "Runde 8", ids: Array.from({ length: 4 }, (_, i) => i + 241) },
    { label: "Runde 9", ids: Array.from({ length: 2 }, (_, i) => i + 245) },
    { label: "Runde 10", ids: Array.from({ length: 2 }, (_, i) => i + 249) },
    { label: "Runde 11", ids: [251] },
    { label: "Finale Verliererseite", ids: [253] },
  ]
  ...
  return (
    <section className="mt-5 rounded-[26px] border border-rose-300/10 bg-rose-400/[0.018] p-5">
      ...
      <div className="mt-5 grid gap-5 xl:grid-cols-6 2xl:grid-cols-12">
        {stages.map((stage) => (
...
function FinalsView({ matches }: { matches: Record<number, DkoMatch> }) {
  const finalMatch = matches[254]
  const resetMatch = matches[255]
...
export default function DkoBeamerPage() {
  ...
  const tournamentName = searchParams.get("tournamentName") || "128er DKO"
  const tournamentType = searchParams.get("tournamentType") || "128er_dko"
  ...
  const totalMatches = 255
  const progress = Math.min(100, Math.round((completedMatches.length / totalMatches) * 100))
  const winner =
    matches[255]?.winner ||
    (matches[254]?.winner && matches[254]?.winner === matches[254]?.player1 ? matches[254]?.winner : undefined)

  const hasLosersView = [65,66,67,68,69,70,71,72,73,74,75,76,77,78,79,80,81,82,83,84,85,86,87,88,89,90,91,92,93,94,95,96,129,130,131,132,133,134,135,136,137,138,139,140,141,142,143,144,145,146,147,148,149,150,151,152,153,154,155,156,157,158,159,160,161,162,163,164,165,166,167,168,169,170,171,172,173,174,175,176,193,194,195,196,197,198,199,200,201,202,203,204,205,206,207,208,209,210,211,212,213,214,215,216,225,226,227,228,229,230,231,232,233,234,235,236,241,242,243,244,245,246,249,250,251,253].some((id) => {
    const match = matches[id]
    return Boolean(match && ((match.player1 && match.player2) || match.winner || match.machineNumber))
  })
  const hasFinalsView = [254, 255].some((id) => {
    const match = matches[id]
    return Boolean(match && ((match.player1 && match.player2) || match.winner || match.machineNumber))
  })
                  ...
                  {[
                    ...
                    const fallbackName =
                      slot.place === 1
                        ? winner
                        : slot.place === 2
                          ? (matches[255]?.loser || matches[254]?.loser || "")
                          : (matches[253]?.loser || "")
