...
function WinnersView({ matches }: { matches: Record<number, DkoMatch> }) {
  ...
  const r2 = pick([5, 6])
  const r3 = pick([7])
  const count = r1.length + r2.length + r3.length
  ...
  return (
    <section className="mt-5 rounded-[26px] border border-sky-300/10 bg-sky-400/[0.018] p-5">
      ...
      <div className="mt-5 grid gap-5 xl:grid-cols-[1.25fr_.9fr_.7fr]">
        <StageColumn label="Runde 1" matches={r1} side="winner" />
        <StageColumn label="Runde 2" matches={r2} side="winner" />
        <StageColumn label="Finale Gewinnerseite" matches={r3} side="winner" />
      </div>
    </section>
  )
}
  ...
  return (
    <section className="mt-5 rounded-[26px] border border-rose-300/10 bg-rose-400/[0.018] p-5">
      ...
      <div className="mt-5 grid gap-5 xl:grid-cols-4">
        ...
        <StageColumn label="Finale Verliererseite" matches={r4} side="loser" />
      </div>
    </section>
  )
}
...
function FinalsView({ matches }: { matches: Record<number, DkoMatch> }) {
  const finalMatch = matches[14]
  const resetMatch = matches[15]
  ...
  return (
    <section className="mt-5 rounded-[26px] border border-amber-300/10 bg-amber-400/[0.018] p-5">
      ...
      <div className="mt-5 grid max-w-[980px] gap-5 md:grid-cols-2">
        {finalReady && (
          <div>
...
export default function DkoBeamerPage() {
  ...
  const tournamentName = searchParams.get("tournamentName") || "8er DKO"
  const tournamentType = searchParams.get("tournamentType") || "8er_dko"
  ...
  const winner =
    matches[15]?.winner ||
    (matches[14]?.winner && matches[14]?.winner === matches[14]?.player1 ? matches[14]?.winner : undefined)

  const hasLosersView = [8, 9, 10, 11, 12, 13].some((id) => {
    const match = matches[id]
    return Boolean(match && ((match.player1 && match.player2) || match.winner || match.machineNumber))
  })
  const hasFinalsView = [14, 15].some((id) => {
    const match = matches[id]
    return Boolean(match && ((match.player1 && match.player2) || match.winner || match.machineNumber))
  })
    ...
    return (
      <div className="grid min-h-screen place-items-center bg-slate-950 px-6 text-white">
        <div className="max-w-lg rounded-[28px] border border-white/10 bg-white/5 p-8 text-center">
          ...
          <p className="mt-2 text-slate-400">Öffne den Beamer direkt aus dem laufenden 8er-DKO.</p>
        </div>
      </div>
    )
  }
                  ...
                  {[
                    ...
                    const fallbackName =
                      slot.place === 1
                        ? winner
                        : slot.place === 2
                          ? (matches[15]?.loser || matches[14]?.loser || "")
                          : (matches[13]?.loser || "")
